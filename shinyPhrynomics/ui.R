...
shinyUI(pageWithSidebar(
  ...
  sidebarPanel(
    ...
    fileInput("SNPdataset", "Choose File To Upload", accept=c("text/csv", "text/comma-separated-values", "text/plain", ".txt")),
    numericInput("obs", "Number Of Taxa to Preview:", 5),
    #numericInput("nloci", "Number Of Loci to Preview:", 2),
    checkboxInput("rmInvSites", "Remove Invariant Sites", value=TRUE),
    checkboxInput("rmNonBin", "Remove Non-Binary Sites", value=TRUE),
    checkboxInput("takeRandom", "Take a single SNP from Each Locus", value=FALSE),
    br(),
  ...
  mainPanel(
    ...
    downloadButton("downloadData", label="Download Modified Results"))
