...
#' @export
#' @return Returns a list of two items. The first item is a matrix of frequencies. The individual values are based on present/absent status. If a informational base (non-missing data) exists, then the frequency of present data is entered here.  For example, if all of the species in a SNP have data, then 1 will be entered for each one. Which base is present does not matter, only that there is or is not data present. This value is the same as the probability that the species contains data for that SNP.  The second item are the row/species means.  This value equates with the probability of an SNP overlapping with another species in the dataset. 
